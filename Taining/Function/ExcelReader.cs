...
namespace Taining.Function
{
    ...
    public static class ExcelReader
    {
        public static string ReadExcelDataAsJson(string filePath)
        {
            ...
            {
                var timeText = ws.Cells[row, 7].Text.Trim();
                if (!string.IsNullOrEmpty(timeText) && !double.TryParse(timeText, out _))
                {
                    errors.Add($"第 {row} 列的『執行時間（Time）』必須為空或是數字，目前值：'{timeText}'");
                }
                else
                {
            ...
            // 如果有錯誤就 MessageBox 並回傳空陣列
            if (errors.Count > 0)
            {
                System.Windows.Forms.MessageBox.Show(
                    "檔案格式錯誤：\n" + string.Join("\n", errors),
                    "Excel 內容錯誤", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Error
                );
                return "[]";
            }
